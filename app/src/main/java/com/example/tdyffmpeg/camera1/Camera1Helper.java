package com.example.tdyffmpeg.camera1;
...
/**
 * camera1相机辅助类，和 CameraListener 共同使用，获取nv21数据等操作
 */
    ...
    public synchronized void focusAtPoint(float x, float y, float radius, final Camera.AutoFocusCallback callback) {
        ...
        if(mCameraParameters.getMaxNumMeteringAreas() > 0) {
            ...
            try {
                ...
                MyLogUtil.e(TAG, "Error: focusAtPoint failed: " + e.toString());
            }
        } else {
            MyLogUtil.e(TAG, "The device does not support metering areas...");
            try {
                mCamera.autoFocus(callback);
            } catch (Exception e) {
                MyLogUtil.e(TAG,  "Error: focusAtPoint failed: " + e.toString());
...
//            LogUtils.i("supportPreviewFps %d - %d", fpsRange[0], fpsRange[1]);
    ...
    private void setPreviewFrameRate(){
        List<Integer> frameRates = mCameraParameters.getSupportedPreviewFrameRates();

        int fpsMax = 0;

        for(Integer n : frameRates) {
            MyLogUtil.e(TAG, "Supported frame rate: " + n);
            if(fpsMax < n) {
                fpsMax = n;
            }
        }
        mCameraParameters.setPreviewFrameRate(fpsMax);
    }
    /**
