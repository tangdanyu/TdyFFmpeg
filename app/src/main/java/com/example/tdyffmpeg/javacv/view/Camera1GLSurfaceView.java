...
public class Camera1GLSurfaceView extends GLSurfaceView implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener {
    ...
    private Queue<Camera1GLSurfaceView.CachedFrame> mFrameQueue;

    private Camera1GLSurfaceView.CachedFrame makeFrame(IplImage img) {
        return new Camera1GLSurfaceView.CachedFrame(img, System.currentTimeMillis(), System.nanoTime());
    }
    ...
    private Camera1GLSurfaceView.CachedFrame getCachedFrame() {
        synchronized (mFrameQueue) {
    ...
    //回收使用过的缓存帧
    private void recycleCachedFrame(Camera1GLSurfaceView.CachedFrame frame) {
    ...
    class RecordingRunnable implements Runnable {
        ...
        public void run() {
            while(mShouldRecord)
            {
                Camera1GLSurfaceView.CachedFrame frame = getCachedFrame();
    ...
    Camera1GLSurfaceView.RecordingRunnable mRecordingRunnable;
    ...
    public Camera1GLSurfaceView.ClearColor clearColor;
    ...
    public synchronized void setFrameRenderer(final Camera1GLSurfaceView.FilterButtons filterID) {
    ...
    public synchronized void startRecording(String filePath) {
        ...
        mRecordingRunnable = new Camera1GLSurfaceView.RecordingRunnable();
    ...
    public Camera1GLSurfaceView(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        clearColor = new Camera1GLSurfaceView.ClearColor();
