package com.example.tdyffmpeg.javacv.recorder;
...
import static org.bytedeco.ffmpeg.global.avcodec.AV_PICTURE_STRUCTURE_FRAME;
...
import static org.bytedeco.ffmpeg.global.avcodec.avpicture_fill;
import static org.bytedeco.ffmpeg.global.avcodec.avpicture_get_size;
...
public class MyRecorder extends FrameRecorder {
    public static MyRecorder createDefault(File f, int w, int h) throws Exception {
        return new MyRecorder(f, w, h);
    }

    public static MyRecorder createDefault(String f, int w, int h) throws Exception {
        return new MyRecorder(f, w, h);
    }
        ...
        if (loadingException != null) {
            ...
            try {
                ...
                if (t instanceof Exception) {
                    ...
                    throw loadingException = new Exception("Failed to load " + MyRecorder.class, t);
                }
            }
        }
    }
    ...
    public MyRecorder(File file, int audioChannels) {
        this(file, 0, 0, audioChannels);
    }

    public MyRecorder(String filename, int audioChannels) {
        this(filename, 0, 0, audioChannels);
    }

    public MyRecorder(File file, int imageWidth, int imageHeight) {
        this(file, imageWidth, imageHeight, 0);
    }

    public MyRecorder(String filename, int imageWidth, int imageHeight) {
        this(filename, imageWidth, imageHeight, 0);
    }

    public MyRecorder(File file, int imageWidth, int imageHeight, int audioChannels) {
        this(file.getAbsolutePath(), imageWidth, imageHeight);
    }

    public MyRecorder(String filename, int imageWidth, int imageHeight, int audioChannels) {
    ...
    public void startUnsafe() throws Exception {
        ...
        if (audioChannels > 0 && audioBitrate > 0 && sampleRate > 0) {
            ...
            if (sampleFormat != AV_SAMPLE_FMT_NONE) {
                audio_c.sample_fmt(sampleFormat);
                    ...
                    (audio_codec.capabilities() & AV_CODEC_CAP_EXPERIMENTAL) != 0) {
        ...
        if (video_st != null) {
            ...
            video_outbuf = null;
            if ((oformat.flags() & AV_PICTURE_STRUCTURE_FRAME) == 0) {
    ...
    public boolean record(int sampleRate, Buffer... samples) throws Exception {
        ...
        while (true) {
            int inputCount = (int) ((samples_in[0].limit() - samples_in[0].position()) / (inputChannels * inputDepth));
            int outputCount = (int) ((samples_out[0].limit() - samples_out[0].position()) / (outputChannels * outputDepth));
            ...
            if (samples_out[0].position() >= samples_out[0].limit()) {
                ...
                avcodec_fill_audio_frame(frame, audio_c.channels(), outputFormat, samples_out[0], (int) samples_out[0].limit(), 0);
                for (int i = 0; i < samples_out.length; i++) {
                    frame.data(i, samples_out[i].position(0));
                    frame.linesize(i, (int) samples_out[i].limit());
                }
