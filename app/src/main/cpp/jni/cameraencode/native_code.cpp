...
//    jclass cl = env->FindClass("com/example/tdyffmpeg/camera/encode/NativeEncoder");
...
Java_com_example_tdyffmpeg_camera_encode_NativeEncoder_encodeMP4Start(JNIEnv *env, jobject thiz,
                                                                      jstring mp4_path, jint width,
                                                                      jint height) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_tdyffmpeg_camera_encode_NativeEncoder_encodeMP4Stop(JNIEnv *env, jobject thiz) {
    if (NULL != videoPublisher) {
        videoPublisher->EncodeStop();
        videoPublisher = NULL;
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_tdyffmpeg_camera_encode_NativeEncoder_onPreviewFrame(JNIEnv *env, jobject thiz,
                                                                      jbyteArray yuv_data,
                                                                      jint width, jint height) {
    if (NULL != videoPublisher && videoPublisher->isTransform()) {
        jbyte *yuv420Buffer = env->GetByteArrayElements(yuv_data, 0);
        videoPublisher->EncodeBuffer((unsigned char *) yuv420Buffer);
        env->ReleaseByteArrayElements(yuv_data, yuv420Buffer, 0);
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_tdyffmpeg_camera_encode_NativeEncoder_encodeJPEG(JNIEnv *env, jobject thiz,
                                                                  jstring jpeg_path, jint width,
                                                                  jint height) {
    if (NULL == jpegEncoder) {
        const char *jpegPath = env->GetStringUTFChars(jpeg_path, NULL);
        jpegEncoder = new JPEGEncoder(jpegPath, width, height);
        env->ReleaseStringUTFChars(jpeg_path, jpegPath);
    }
}